#-------------------------------------------------------------------------------
# Name:        Colors
# Purpose:     A list of common colors
#
# Author:      gehmd
#
# Created:     25/04/2019
...
def getLuminance(color):
    """Return the luminance value for a color, the luminance is how 'bright' the human eye percieves a color"""

    #(color[0] * 0.299 + color[1] * 0.587 + color[2] * 0.117) # l between 0 and 255
    #(color[0] * 0.2126 + color[1] * 0.7152 + color[2] * 0.0722) # l between 0 and 255
    #math.sqrt(color[0] ** 2 * 0.299 + color[1] ** 2 * 0.587 + color[2] ** 2 * 0.114) # l between 0 and 255
    return (color[0] * 0.299 + color[1] * 0.587 + color[2] * 0.114);

def getColors():
    """Returns a list of all the names of the colors in this module"""

    colorList = [name for name, var in globals().items() if not name.startswith("__") and isinstance(var, tuple)];

    return colorList;

def getDarkColors(threshold = 127):
    """Returns a list of all the names of colors with less luminance than the threshold"""

    darkCol = lambda col: getLuminance(col) < threshold;

    colorList = [name for name, var in globals().items() if not name.startswith("__") and isinstance(var, tuple) and darkCol(var)];

    return colorList;

def getLightColors(threshold = 129):
    """Returns a list of all the names of colors with more luminance than the threshold"""

    lightCol = lambda col: getLuminance(col) > threshold;

    colorList = [name for name, var in globals().items() if not name.startswith("__") and isinstance(var, tuple) and lightCol(var)];

    return colorList;

def getColor(colorName):
    """Returns a 4 tuple containing the RGBA color values for the given color name

    colorName is a string containing the name of a color from this Module
    """
...
if __name__ == "__main__":
    ...
    print(len(colors), "colors available");
    #print("colors are", ', '.join(colors));
    print("There are", len(darkColors), "dark colors.");
    print("There are", len(lightColors), "light colors.");
    print("SPRING_GREEN is ", getColor("SPRING_GREEN"));
    print("The luminance of SPRING_GREEN is", getLuminance(SPRING_GREEN));
    pass;